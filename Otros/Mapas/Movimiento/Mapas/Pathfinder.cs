...
namespace Bot_Dofus_Retro.Otros.Mapas.Movimiento.Mapas
{
    public class Pathfinder : IDisposable
    {
        ...
        public List<Celda> get_Path(Celda celda_inicio, Celda celda_final, List<Celda> celdas_no_permitidas, bool detener_delante, byte distancia_detener, bool utilizar_diagonales)
        {
            ...
            while (celdas_permitidas.Count > 0)
            {
                ...
                foreach (Celda celda_siguiente in get_Celdas_Adyecentes(actual, utilizar_diagonales))
                {
