...
namespace Bot_Dofus_Retro.Otros.Game.Entidades.Manejadores.Recolecciones
{
    public class Recoleccion : IDisposable
    {
        ...
        public bool get_Intentar_Recolectar(List<short> elementos)
        {
        ...
        private Dictionary<short, ObjetoInteractivo> get_Interactivos_Utilizables(List<short> elementos_ids)
        {
            ...
            foreach (ObjetoInteractivo interactivo in mapa.interactivos.Values.OrderBy(x => x.celda.get_Distancia(personaje.celda)))
            {
                ...
                List<Celda> path = pathfinder.get_Path(personaje.celda, interactivo.celda, mapa.celdas_ocupadas(), true, distancia_arma, personaje.derechos.PUEDE_MOVERSE_TODAS_DIRECCIONES);
                ...
                foreach (short habilidad in interactivo.modelo.habilidades)
                {
                    ...
                    if (!es_herramienta_pescar && path.Last().get_Distancia(interactivo.celda) > 1)
                        continue;

                    if (es_herramienta_pescar && path.Last().get_Distancia(interactivo.celda) > distancia_arma)
                        continue;
            ...
            if (!robado)
            {
                foreach (short habilidad in interactivo_recolectando.modelo.habilidades)
                {
                    if (cuenta.juego.personaje.oficios.get_Skills_Recoleccion_Disponibles().Contains(habilidad))
                        await cuenta.conexion.enviar("GA500" + interactivo_recolectando.celda.id + ";" + habilidad);
                }
            }
            else
