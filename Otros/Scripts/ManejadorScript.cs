...
using Bot_Dofus_Retro.Otros.Scripts.Acciones.Almacenamiento;
using Bot_Dofus_Retro.Otros.Scripts.Acciones.Global;
using Bot_Dofus_Retro.Otros.Scripts.Acciones.Npc;
using Bot_Dofus_Retro.Otros.Scripts.Api;
using Bot_Dofus_Retro.Otros.Scripts.Banderas;
...
namespace Bot_Dofus_Retro.Otros.Scripts
{
    public class ManejadorScript : IDisposable
    {
        private Cuenta cuenta;
        private LuaManejadorScript _manejador_script;
        public ManejadorAcciones manejar_acciones { get; private set; }
        private EstadoScript _estado_script;
        ...
        public bool corriendo => cuenta?.es_lider_grupo == true ? activado && !pausado : cuenta?.grupo.lider.script.corriendo == true;
        public LuaManejadorScript manejador_script => cuenta.es_lider_grupo ? _manejador_script : cuenta.grupo.lider.script._manejador_script;
        private EstadoScript estado_script
        {
            get => cuenta.es_lider_grupo ? _estado_script : cuenta.grupo.lider.script._estado_script;
            set
            {
                if (cuenta.es_lider_grupo)
                    _estado_script = value;
                else
                    cuenta.grupo.lider.script._estado_script = value;
            }
        }
        ...
        {
            cuenta = _cuenta;
            _manejador_script = new LuaManejadorScript();
        ...
        public void detener_Script(string mensaje = "script detenido")
        {
            if (cuenta.tiene_grupo && !cuenta.es_lider_grupo && cuenta.grupo.lider.script.corriendo)
            {
                cuenta.grupo.lider.script.detener_Script(mensaje);
                return;
            }
        ...
        private void iniciar_Script() => Task.Run(async () =>
        {
            ...
            try
            {
                if (cuenta.tiene_grupo && cuenta.es_lider_grupo)
                {
                    await cuenta.grupo.get_Grupo_Necesita_Regruparse();

                    if (cuenta.grupo.get_Grupo_Esta_ocupado())
                    {
                        detener_Script("Un miembro del grupo esta ocupado");
                        return;
                    }
                }
                ...
                if (cuenta.tiene_grupo && cuenta.es_lider_grupo)
                    await cuenta.grupo.get_Comprobaciones_Script();
                else
        ...
        public async Task aplicar_Comprobaciones()
        {
        ...
        private async Task verificar_Muerte()
        {
            ...
            if (cuenta.juego.personaje.restricciones.ES_FANTASMA)
            {
                cuenta.logger.log_informacion("SCRIPT", "Pasando a modo fénix");
        ...
        private RecoleccionAccion crear_Accion_Recoleccion()
        {
            ...
            if (elementos_para_recolectar != null)
            {
                foreach (DynValue etg in elementos_para_recolectar.Values)
                {
                    ...
                    if (cuenta.juego.personaje.oficios.get_Tiene_Skill_Id((int)etg.Number))
                        recursos_id.Add((short)etg.Number);
                }
            }

            if (recursos_id.Count == 0)
                recursos_id.AddRange(cuenta.juego.personaje.oficios.get_Skills_Recoleccion_Disponibles());
        ...
        {
            manejar_acciones.enqueue_Accion(new NpcBanco(-1, -1));
