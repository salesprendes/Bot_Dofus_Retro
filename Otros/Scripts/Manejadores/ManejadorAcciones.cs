...
using Bot_Dofus_Retro.Utilidades;
...
namespace Bot_Dofus_Retro.Otros.Scripts.Manejadores
{
    public class ManejadorAcciones : IDisposable
    {
        ...
        private void npcs_Respuestas_Recibido()
        {
            if (!cuenta.script.corriendo)
                return;
        ...
        private void npcs_Dialogo_Acabado()
        {
            ...
            if (accion_actual is ResponderDialogo || accion_actual is CerrarVentanaAccion)
                acciones_Salida(200);
        }

        private void get_Interactivo_Finalizado(bool es_correcto)
        {
            if (!cuenta.script.corriendo)
                return;

            if (accion_actual is UtilizarInteractivoAccion)
            {
                if (!es_correcto)
                    cuenta.script.detener_Script();
                else
                    acciones_Salida(fila_acciones.Count > 0 ? 0 : 500);
            }
        }
        ...
        public void enqueue_Accion(AccionesScript accion, bool iniciar_dequeue_acciones = false)
        {
            ...
            if (cuenta.tiene_grupo && cuenta.es_lider_grupo)
                cuenta.grupo.enqueue_Acciones_Miembros(accion, iniciar_dequeue_acciones);
        }
